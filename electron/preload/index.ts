...
const api = {
  ...
  getTrending: (forceRefresh = false) =>
    invoke<TrendingResponse>(IPC.TRENDING_GET, forceRefresh),
  getSuggestions: (query: string, limit = 8) =>
    invoke<string[]>(IPC.SUGGEST_GET, query, limit),
  ...
  downloadUpdate: () => invoke<boolean>(IPC.UPDATE_DOWNLOAD),

  getUpdateState: () =>
    invoke<UpdateStatusSnapshot>(IPC.UPDATE_STATE_GET),
