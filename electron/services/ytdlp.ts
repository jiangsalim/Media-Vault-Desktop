...
function binArgsBase(): { bin: string; common: string[] } {
  ...
  // yt-dlp needs a JS runtime (Deno) to solve YouTube's player challenges.
  const jsRuntimeArgs: string[] = [];
  const denoPath = resolveBinaryPathOrNull('deno');
  if (denoPath) {
    jsRuntimeArgs.push('--js-runtimes', `deno:${denoPath}`);
  }
...
/**
 * Fast YouTube search via yt-dlp's `ytsearch` extractor with --flat-playlist.
 * Returns lightweight results (id, title, channel, duration, thumbnail) without
 * per-video format metadata. Users click a result → the normal analyzeUrl()
 * pipeline runs on that URL to fetch full details.
 */
export async function searchYoutube(
  ...
  // Cap between 1 and 50 to prevent abuse and runaway requests.
  ...
  log.info('search', `ytsearch${n}: ${trimmed}`);
  ...
  try {
    ...
    return entries.map((e) => {
      // Flat-playlist entries often lack thumbnails; derive from id when possible.
      const thumb =
